...
When the docker runner is active on a Linux host, scrutineer creates an `--internal` docker bridge network (`scrutineer-egress` by default, override with `egress_network`) and attaches each scan container to it. Containers on that network have no route to the outside world; the only reachable address is the host's bridge interface, where scrutineer's authenticated egress proxy listens. `HTTPS_PROXY`/`HTTP_PROXY` inside the container point at that proxy, and a process that ignores those variables hits "network unreachable" rather than the open internet. On Docker Desktop (macOS, Windows) the internal network can't reach the host process, so containers stay on the default bridge and the proxy is cooperative only; scrutineer logs a warning at startup. The proxy only tunnels to an allowlist of hosts: the Anthropic API, `*.ecosyste.ms`, the major forges (GitHub, GitLab, Codeberg, Bitbucket), common package registries (npm, PyPI, RubyGems, crates.io, Go module proxy, Packagist, Hex, NuGet), advisory sources (semgrep.dev, OSV, NVD, cwe.mitre.org), and `host.docker.internal` for the local skill API. Requests to anything else get a 403 and are logged. Extend the list with `egress_allow` in the config file; carve hosts back out of a wildcard with `egress_deny`. Cloud metadata endpoints (`169.254.169.254`, `metadata.google.internal`, ...) are always denied regardless of allowlist. When `-anthropic-base-url` is set (or falls back to the `ANTHROPIC_BASE_URL` env var), its hostname is automatically added to the allowlist. The proxy uses a per-process random token so it isn't an open relay on the LAN.
