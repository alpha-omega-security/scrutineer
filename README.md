...
When the docker runner is active, scrutineer creates an `--internal` docker bridge network (`scrutineer-egress` by default, override with `egress_network`) and starts a long-lived `scrutineer-egress-proxy` container connected to both that network and the default bridge. Scan containers attach only to the internal network: they have no route to the outside world, and the proxy container is the only thing they can reach. `HTTPS_PROXY`/`HTTP_PROXY` inside each scan point at the proxy by container name; a process that ignores those variables hits "network unreachable" rather than the open internet. The proxy only tunnels to an allowlist of hosts: the Anthropic API, `*.ecosyste.ms`, the major forges (GitHub, GitLab, Codeberg, Bitbucket), common package registries (npm, PyPI, RubyGems, crates.io, Go module proxy, Packagist, Hex, NuGet), advisory sources (semgrep.dev, OSV, NVD, cwe.mitre.org), and `host.docker.internal` for the local skill API. Requests to anything else get a 403 and are logged in scrutineer's output. Extend the list with `egress_allow` in the config file; carve hosts back out of a wildcard with `egress_deny`. Cloud metadata endpoints (`169.254.169.254`, `metadata.google.internal`, ...) are always denied regardless of allowlist. When `-anthropic-base-url` is set (or falls back to the `ANTHROPIC_BASE_URL` env var), its hostname is automatically added to the allowlist. The proxy uses a per-process random token so requests from other containers on the internal network (or anything that finds the port) get a 407.
