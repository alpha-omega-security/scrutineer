...
When the container runner is active, scrutineer starts an authenticated egress proxy on the host and points `HTTPS_PROXY`/`HTTP_PROXY` inside the container at it. The proxy only tunnels to an allowlist of hosts: the active model API, `*.ecosyste.ms`, the major forges (GitHub, GitLab, Codeberg, Bitbucket), common package registries (npm, PyPI, RubyGems, crates.io, Go module proxy, Packagist, Hex, NuGet), advisory sources (semgrep.dev, OSV, NVD, cwe.mitre.org), and the runtime's host endpoint (`host.docker.internal` for docker/podman, the default gateway IP for Apple's `container`) for the local skill API. Requests to anything else get a 403 and are logged. Extend the list with `egress_allow` in the config file. `ecosystems_enrichment: false` (or `-ecosystems-enrichment=false`) removes `*.ecosyste.ms` from that allowlist and stops the enrichment scrutineer's own process performs, so a restricted-egress host declines third-party enrichment in one place and can then deny the domain outright; the `metadata`, `packages` and `advisories` skills get a 403 from the proxy on the fetches they would have made, and SBOM and dependency import report that the package-to-repository resolution is unavailable rather than silently doing nothing. When `-model-base-url` is set (or, for Claude, falls back to the `ANTHROPIC_BASE_URL` env var), its hostname is automatically added to the allowlist. The proxy uses a per-process random token so it isn't an open relay; tools that ignore the proxy env are not blocked at the network layer (see `threatmodel.md`).
...
| `-ecosystems-enrichment` | `true` | Enrich repositories from ecosyste.ms. `=false` stops every lookup scrutineer's own process makes and drops `*.ecosyste.ms` from the container egress allowlist; `--no-container` runs no proxy, so skills that fetch ecosyste.ms directly are unaffected there |
