...
|------|-----------------|---------|
| certificate | `.../scrutineer/interchange/certificate/v1` | An advisory's advertised fix was re-audited on the named repository and held. The web UI serves the same attestation as `GET /advisories/{id}/certificate.json`. |
...
that smuggles one in fails validation.
...
- `cwe` is uppercased and trimmed (empty stays empty).
...
- `404` -- `federation_salt` is not configured; a non-federated instance is
  indistinguishable from one without the endpoint.
...
or `-federation-salt` / `-federation-contact` on the command line. Both
default to empty; an empty salt disables federation.
