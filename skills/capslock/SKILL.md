...
compatibility: Requires the Go runner profile, including Google Capslock and a repository-root go.mod. Analyses packages in the local module and disables Capslock's temporary-module fallback; normal Go module loading may still use the configured module cache or proxy for declared dependencies.
...
does not use Capslock's temporary-module fallback for an arbitrary package
outside this checkout. Capslock's normal Go package loading can still consult
the configured module cache or proxy for dependencies declared by this module.
