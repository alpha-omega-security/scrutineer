...
A sink's input can be absent, not only hostile. When the sink guards a security decision — authentication, authorization or access control, the existence or identity of a session, CSRF validation, a rate-limit or credential check — finish the trace by asking what the code does when that input is missing or empty. The value reaching the sink is, for this purpose, `None`, `Undefined`, the empty string, an absent header, a missing session key, an unset cookie, a `0`/`-1` where an id was expected. The question is which way the decision falls when the lookup yields nothing: does the handler reject the request (fail closed), or does it fall through to the authorised path as if the check had passed (fail open)? A credential read as `request.headers.get("X-API-Key")` and then compared only on equality, a session id fetched with a `.get()` default that the authorised branch never distinguishes from a real session, a CSRF token whose absence skips the equality test, a rate-limit counter that defaults to zero and is never throttled — each is a bypass reached by omitting the input, before the attacker has to forge one. This is distinct from the boundary judgement in Step 2: the input being attacker-controllable in the usual sense does not matter here, because the attacker is not setting the input, they are sending none of it. Trace the absence: name the read primitive (key lookup, header accessor, cookie read), the value it returns on a miss, and the branch each of those sentinel values falls into. If the absent case reaches the authorised or serviced path, the sink fails open; record that branch, the sentinel value, and the line, and carry the bypass into Step 2 as its own finding rather than ruling the sink out as harmless. Do not widen this to inputs that are not security decisions — an optional `?format=json` query parameter, a missing cosmetic header, an unset non-security feature flag has no privileged branch to fall into. Developer-only entry points and trusted entry points named in the threat model keep their existing judgement: a missing console flag on a dev-only path does not become a bypass because the threat model already scopes that boundary out.
