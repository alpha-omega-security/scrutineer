...
### T13: Runner egress (mitigated on Linux, cooperative on Docker Desktop)

On a Linux host, scan containers attach to an `--internal` docker bridge network (`scrutineer-egress`, configurable via `egress_network`) created on startup. The internal flag means docker installs no MASQUERADE rule and adds isolation rules that drop traffic from the bridge to any other interface, so containers have no route to the outside world. The only reachable address is the host's interface on that bridge, where the allowlisting CONNECT/forward proxy listens. `HTTPS_PROXY`/`HTTP_PROXY` in the container point at it, and a process that ignores those variables gets ENETUNREACH rather than open egress. `enable_icc=false` on the network stops concurrent scan containers reaching each other. The proxy 403s anything off the allowlist (Anthropic, ecosyste.ms, forges, registries, advisory sources, the local skill API), hard-denies cloud metadata endpoints and link-local IPs regardless of allowlist, and listens on all interfaces with a per-process random `Proxy-Authorization` token so it isn't an open relay on the LAN. The `host.docker.internal` → `127.0.0.1` rewrite is gated behind the same token so the loopback-bound web UI stays off the LAN.

On Docker Desktop (macOS, Windows) the bridge gateway IP belongs to the Linux VM rather than the host process, so an `--internal` network would cut the container off from the proxy too. Scrutineer detects this at startup, logs a warning, and falls back to the default bridge with the cooperative proxy: a process that ignores `HTTPS_PROXY` can still dial out. Closing this gap on Desktop means running the proxy as a container attached to both the internal network and the default bridge; tracked under #3.

Residual on all platforms: `ANTHROPIC_API_KEY` is still passed into the container environment, so a hijacked agent that exfiltrates it through an allowlisted host (a gist, an npm package publish) can still leak the key. Moving the key out of the container and having the proxy inject it on the upstream Anthropic connection is tracked under #3. The proxy also does not resolve and re-check CONNECT targets after DNS, so an attacker who controls an allowlisted hostname's DNS could point it at a private IP; the metadata/link-local hard-deny covers the IMDS case but not arbitrary internal services.
