...
### T13: Runner egress (mitigated)

Scan containers attach only to an `--internal` docker bridge network (`scrutineer-egress`, configurable via `egress_network`). The internal flag means docker installs no MASQUERADE rule and adds isolation rules that drop traffic from the bridge to any other interface, so scan containers have no route to the outside world; a process that ignores `HTTPS_PROXY` gets ENETUNREACH. The egress proxy runs as its own container (`<network>-proxy`, `cmd/scrutineer-proxy`) attached to both the internal network and the default bridge: scan containers reach it by name on the internal side, it dials upstreams on the bridge side. This holds on native Linux and Docker Desktop alike.

The proxy 403s anything off the allowlist (Anthropic, ecosyste.ms, forges, registries, advisory sources, the local skill API), hard-denies cloud metadata endpoints and link-local IPs regardless of allowlist, and requires a per-process random `Proxy-Authorization` token. It rewrites `host.docker.internal` to the host gateway and forwards to a second bearer-auth'd `/api`-only listener that scrutineer binds on all interfaces; the web UI listener stays on loopback.

Residual: `ANTHROPIC_API_KEY` is still passed into scan container environments, so a hijacked agent that exfiltrates it through an allowlisted host (a gist, an npm package publish) can still leak the key; moving the key out of the container and having the proxy inject it on the upstream Anthropic connection is the next step under #3. Concurrent scan containers share the internal network and can address each other (ICC stays on so scans can reach the proxy), though none of them listen on anything. The skill-API listener on 0.0.0.0 is reachable from the LAN but every route requires a per-scan bearer that only exists while that scan runs. The proxy does not resolve and re-check CONNECT targets after DNS, so an attacker who controls an allowlisted hostname's DNS could point it at a private IP; the metadata/link-local hard-deny covers the IMDS case but not arbitrary internal services.
